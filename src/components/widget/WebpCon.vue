...
import { NSpace, NSlider, NProgress, NPopover, NInputNumber, NIcon } from 'naive-ui'
...
const webpOpen = () => {
    userStore.toWebp.active = !userStore.toWebp.active
},
    quality = computed(() => userStore.toWebp.quality)

watchEffect(() => {
    userStore.toWebp.active ?
        webpActive.value = statusOn :
        webpActive.value = statusOff
})
watch(quality, () => userStore.toWebp.active = true)
</script>


<template>
    <div class="webp-box" v-show="userStore.toWebp.open">
        <n-popover trigger="hover" :show-arrow="false">
            <template #trigger>
                <div class="progress-con" style="cursor:pointer;height: 22px;" @click="webpOpen">
                    <n-progress style="width: 22px;" type="circle" :percentage="quality"
                        :color="webpActive.c" :rail-color="webpActive.rc" :indicator-text-color="webpActive.tc"
                        :stroke-width="10" />
                </div>
            </template>
            <div class="widget-webp" style="width: 200px;">
                <n-space vertical>
                    <n-slider v-model:value="userStore.toWebp.quality" :step="1" :min="15" :max="99" :tooltip="false" />
                </n-space>
                <div class="webp-i">
                    ...
                    <n-input-number :min="15" :max="99" v-model:value="userStore.toWebp.quality" />
                </div>
            </div>
        </n-popover>
    </div>
